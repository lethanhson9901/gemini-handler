...
model_list, api_key_map = auto_generate_model_list("../config.yaml")
router = TrackedRouter(model_list=model_list, api_key_map=api_key_map)

# Make 3 calls to test load balancing and proxy rotation
for i in range(3):
    print(f"\n--- Call #{i+1} ---")
    ...
    try:
        response = router.completion(
            ...
            messages=[{"role": "user", "content": f"Write a very short poem about AI (call {i+1})"}],
            temperature=0.7
        )
